            ...
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd
                  ...
                  @click="
                    checkAttrValue(saleAttrValue, saleAttr.spuSaleAttrValueList)
                  "
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="skuNum"
                  @change="queryInputValue"
                />
                <a class="plus" @click="addValue">+</a>
                <a class="mins" @click="reduceValue">-</a>
              </div>
              <div class="add">
                <!-- 需要传参过去，不是单一的路由跳转 -->
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // 数据未返回时，skuInfo这是个空对象
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    // 改变选择属性值的边框样式
    checkAttrValue(saleAttrValue, arr) {
      arr.forEach((item) => {
        item.isChecked = "0";
      });
    ...
    // 加数量操作
    addValue() {
      // console.log("加", this.$refs.num.value);
      // this.$refs.n.value++;
      this.skuNum++;
    },
    // 减数量操作
    reduceValue() {
      // console.log("减",this.skuNum);
      /* this.skuNum--;
      if (this.skuNum <= 0) {
        this.skuNum = 1;
      } */
    ...
    queryInputValue() {
      ...
      /* if (isNaN(this.skuNum) || this.skuNum <= 0) {
        this.skuNum = 1;
      } else {
        this.skuNum = parseInt(this.skuNum);
      } */
      this.skuNum =
        isNaN(this.skuNum) || this.skuNum <= 0 ? 1 : parseInt(this.skuNum);
    },
    // 点击【添加购物车】获取信息
    async addOrUpdateShopCart() {
      try {
        ...
        // 使用会话|本地存储传递复杂数据，但是只能传递字符串，不能传递对象
